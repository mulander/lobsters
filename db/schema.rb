...
ActiveRecord::Schema[7.0].define(version: 2022_08_06_200248) do
  ...
  create_table "comments", id: { type: :bigint, unsigned: true }, charset: "utf8mb4", force: :cascade do |t|
    ...
    t.index ["comment"], name: "index_comments_on_comment", using: :gin, opclass: :gin_trgm_ops
  ...
  create_table "stories", id: { type: :bigint, unsigned: true }, charset: "utf8mb4", force: :cascade do |t|
    ...
    t.index ["description"], name: "index_stories_on_description", using: :gin, opclass: :gin_trgm_ops
    ...
    t.index ["title"], name: "index_stories_on_title", using: :gin, opclass: :gin_trgm_ops
  ...
  create_table "story_texts", charset: "utf8mb4", force: :cascade do |t|
    ...
    t.index ["body"], name: "index_story_texts_on_body", using: :gin, opclass: :gin_trgm_ops
  end
  ...
  create_view "replying_comments", sql_definition: <<-SQL
      ...
      (select "votes"."vote"
        ...
        where "votes"."user_id" = "read_ribbons"."user_id"
          and "votes"."comment_id" = "comments"."id") AS "current_vote_vote",
      (select "votes"."reason"
        from "votes"
        where "votes"."user_id" = "read_ribbons"."user_id"
          and "votes"."comment_id" = "comments"."id") AS "current_vote_reason"
      from (
        (("read_ribbons"
      ...
      where "read_ribbons"."is_following" is True
        and "comments"."user_id" <> "read_ribbons"."user_id"
        and "comments"."is_deleted" is False
        and "comments"."is_moderated" is False
        ...
        and ("parent_comments"."id" is null
              or "parent_comments"."score" >= 0
              and "parent_comments"."is_moderated" is False
              and "parent_comments"."is_deleted" is False)
        and not exists(
            select 1
              from ("votes" "f"
              ...
              where "f"."vote" < 0
                ...
                and "f"."story_id" = "comments"."story_id" limit 1)
  SQL
end
